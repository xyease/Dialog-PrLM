...
from RSmatching_model_2_electra import RSmatching_model
...
parser.add_argument("--output_dir",
                    default="../../result/alime/RSmatch2_v3_electra_base_zh_wikipretrain_3_p2",
                    type=str,
...
parser.add_argument("--input_cache_dir",
                    default="input_RSmatch2_v3_electra_base_zh_wikipretrain_3_p2",
                    type=str,
...
def main():
    ...
    if args.do_train:
        ...
        output_model_file = os.path.join(args.output_dir,  WEIGHTS_NAME)
        model_state_dict = torch.load(output_model_file)
        model = RSmatching_model.from_pretrained(args.bert_model, config=config, state_dict=model_state_dict, device=device)
        model.resize_token_embeddings(len(tokenizer))
        model.to(device)
        if n_gpu > 1:
            model = torch.nn.DataParallel(model)
