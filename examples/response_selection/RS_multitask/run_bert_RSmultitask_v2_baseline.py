...
parser.add_argument("--output_dir",
                    default="../../result/douban/RSmatch2_v3_wikipretrain_3_p2_multitask_baseline_try2",
                    type=str,
...
def main():
    ...
    if args.do_train:
        ...
        model = Bertmultitaskmodel.from_pretrained(args.bert_model, config=config, state_dict=model_state_dict)
        model.resize_token_embeddings(len(tokenizer))
        model.to(device)
        if n_gpu > 1:
            model = torch.nn.DataParallel(model)
